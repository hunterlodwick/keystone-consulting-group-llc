...
const stage=process.argv[2] || 'after', root=`verification/services-overhaul/${stage}`;
fs.mkdirSync(root,{recursive:true});

const results=[];
const deep=['web-design','automations'], others=['crm','consulting','prep-to-sell','seo','google-business','bpo','consumer-financing','business-loans','pos-placement'];
for(const id of [...deep,...others]) for(const width of deep.includes(id)?[1440,390]:[1440]) for(const theme of deep.includes(id)?['light','dark']:['light']) {
 ...
 await page.goto(`http://127.0.0.1:4177/services/${id}`,{waitUntil:'networkidle'});
